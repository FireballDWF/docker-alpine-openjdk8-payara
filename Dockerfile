...
ENV PAYARA_PKG https://s3-eu-west-1.amazonaws.com/payara.co/Payara+Downloads/Payara+4.1.1.161/$PKG_FILE_NAME
...
RUN mkdir /opt && adduser -D -s /bin/bash -h /opt/payara payara && echo payara:payara | chpasswd
RUN cd /opt && wget $PAYARA_PKG && unzip $PKG_FILE_NAME && rm $PKG_FILE_NAME && chown -R payara:payara /opt/payara41*
